...
| PO2-SEC-001 | Rotation secrets ENEDIS leakes dans le chat | Todo | P0 | Acces utilisateur au portail ENEDIS | Securite secrets prod | 3 secrets ENEDIS ont fuite en chat sur 3 sessions distinctes (password FTP, cle AES). Apres validation d'un premier fichier dechiffre OK, rotater cote portail ENEDIS, mettre a jour `/root/.ftp_password_enedis` et `.env` sur le VPS, redemarrer backend |
