...
| PO2-FACT-001 | Audit facture ENGIE complet | En cours | P1 | BPU, TURPE, ENEDIS, lot historique ENGIE | Decision facture fiable + historique depenses/tarifs | Rapport fournisseur filtrable : limites internes TURPE/BPU/ENEDIS exclues. Audit local des 83 PDF : corriger/reanalyser en prod, qualifier gaps/overlaps de periode et les 33 ecarts BPU lot 1 restants. |
...
| PO2-PAT-002 | Import patrimoine hierarchique site/batiment/local | En cours | P1 | Fichier inventaire avec colonne Typologie | Import fidele du listing patrimoine et rattachement des locaux | Modele durable implemente : table `sites`, `buildings.site_id`, endpoints `/api/buildings/sites`, import qui cree/reutilise les sites puis rattache batiments et locaux. Reste a tester l'import reel et a ajouter une navigation/filtre par site. |
...
| PO2-METER-001 | Rattachement compteurs fluides aux batiments | En cours | P1 | Referentiel batiments, donnees ENEDIS, futurs GRDF/SUEZ | Fiche batiment centrale, audit conso par batiment, OPERAT | Socle manuel livre : table/API/UI `BuildingMeterLink`. Prochaine etape : rattacher les PRM/PCE aux donnees ENEDIS/GRDF et traiter le cas d'un compteur partage entre plusieurs batiments. |
