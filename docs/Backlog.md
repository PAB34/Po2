...
|---|---|---|---|---|---|---|
| PO2-THERM-001 | Outil de métré thermique (`thermique.patrimoineaucarre.com`) — projet à part, hébergé sur l'infra Po2 | En cours | Projet distinct (pas en concurrence avec Po2) | — | Géométrie des plans puis métrés et ponts thermiques, pour les thermiciens privés en bureau d'études | F0/F1/F2/F3/F4 terminés ; nature des locaux sortie de l'édition du contour et `gaine_technique` livrée localement sur `feat/thermique-socle-raster` (D115-D122), sans régression du R+1. Prochaine action : cadrer puis développer Ctrl+Z / Rétablir ; recette authentifiée et déploiement uniquement après accord explicite. Décisions : `thermique/parcours-F2-decisions.md`, `thermique/nature-locaux-gaine-decisions.md`. |
