...
| PO2-FACT-002 | Controle factures gaz TotalEnergies | En cours | P1 | Fichier export TotalEnergies, BPU gaz lot 7, bareme ATRD/ATRT, taux TICGN | Decision facture gaz fiable + suivi conso/depense gaz Ville | V1 livree (2026-06-22) : module gaz dedie `gas_invoices` (migration 0057), parser xlsx + moteur de coherence (prix×kWh, Σ=HT, HT+TVA=TTC, conversion m³→kWh, TVA), endpoints `/api/gas/invoices/*`, onglet Factures marche > TotalEnergies (import, portefeuille, controle, decision). Import alimente `gas_pces` -> boite de rapprochement. V2 livree (2026-06-22) : controle prix fourniture vs BPU lot 7 (reference editable `gas_bpu_prices`, migration 0058, seed verifie contre `extraction_tarifs_BPU_herault.xlsx` : 35.23/3.89/3.06/0.41/16.25 EUR HT/MWh) ; valide staging = 49 conformes / 9 a prix revisable PEG signales. V3 livree (2026-06-22) : export fiche liaison finance XLSX (`GET /api/gas/invoices/export`, horodate la transmission) + controle de coherence de l'acheminement ATRD (taux variable EUR/MWh median par tarif, ~12.08 en T2, ecart signale, sans donnee externe). Parcours gaz complet (importer->controler->comprendre->decider->exporter). Reste : bareme ATRD/ATRT GRDF absolu (donnee externe a fournir), CEE definitifs, suivi conso vs releves GRDF. |
