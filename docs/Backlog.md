...
| PO2-CVC-001 | Import inventaire materiels CVC terrain | En cours | P1 | `saas/CVC/listing materiels V2.xlsx`, referentiel SYPEMI | Fiche technique batiment, maintenance, BACS futur | Reouvert 2026-06-03 : nouveau flux vise dans `/buildings/cvc-import` = upload persistant de l'inventaire brut, tableau editable, rattachement Site/Batiment/Local patrimoine, attribution ligne par ligne au referentiel `equipment_references` issu de `durees_vie_powerbi_base_wide.csv`, affichage mini/reference/maxi/restant et champ quantite fluide frigorigene seulement pour `Production de froid :` et `Pompes a chaleur Air/Air, Air/Eau, Eau/Eau`. Increment code local ajoute migration 0042 + API batches/items/update ; validation frontend a faire via CI faute de npm local. |
