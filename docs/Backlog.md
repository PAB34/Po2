...
| PO2-FACT-001 | Audit facture ENGIE complet + socle historique EDF | En cours | P1 | BPU, TURPE, ENEDIS, lot historique ENGIE | Decision facture fiable + historique depenses/tarifs | Le controle BPU sait d'abord chercher une reference historique exacte dans `bpu_*` (fournisseur/annee/segment/poste/composante) puis se replier sur `BillingBpuLine`. Prochaine marche : modele de contexte marche explicite + qualification d'un echantillon de factures EDF anciennes. |
