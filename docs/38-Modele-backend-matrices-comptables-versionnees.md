...
### Import/export XLSX (fait — PR #27)

`app/services/accounting_matrix_xlsx.py` + endpoints `GET .../versions/{id}/export.xlsx` (feuille `Matrice` + onglet `Lisez-moi`, round-trip par `stable_rule_key`), `POST .../contracts/{id}/import-preview` (diff sans écriture) et `POST .../contracts/{id}/import-commit` (version brouillon depuis le classeur, sans muter une version active). Import sans état (pas de table `accounting_matrix_import_batches`). Tests : `tests/test_accounting_matrix_xlsx.py`.

### Application à la facture + snapshots (fait — PR #28)

`app/services/accounting_matrix_apply.py` : moteur pur `apply_matrix` + cycle de vie via `POST .../invoices/{source}/{id}/apply` (proposition figée sur la version active, n'écrase pas un snapshot figé), `validate-snapshot` (bloque si exceptions / ventilation incomplète), `manual-override` (correction motivée), `export-finance`. Immutabilité, dédoublonnage et contrôle ventilation couverts par `tests/test_accounting_matrix_apply.py`.

### Différé à la phase suivante

- brancher les **extracteurs réels** de lignes facture par source (energy_import / gas_totalenergies / cpe_dalkia) en entrée de `apply` (aujourd'hui fournies par l'appelant) ;
- droits par rôle (comptabilité / responsable marché / admin, cf. doc 35 §6) ;
