# =============================================================================
# Fabric Notebook — ENEDIS Backfill Complet (exécution unique)
# Workspace : ENEDIS  |  Lakehouse : ENEDIS_LAKEHOUSE
#
# ► Rôle : rattraper l'historique complet (2 ans) pour :
#          - Consommation journalière  (enedis_data.csv)
#          - Courbe de charge 30 min   (enedis_load_curve.csv)
#          Boucle automatiquement par tranches jusqu'à être à jour.
#          Les erreurs 404/400 (données manquantes) sont ignorées — le run continue.
# ► À lancer UNE SEULE FOIS manuellement, puis laisser tourner.
# ► Après la fin du backfill, utiliser 03_sync_enedis_fabric.py (planifié 05:30).
# =============================================================================
...
CHUNK_DAYS       = 7     # jours par itération (court = itérations rapides, checkpoints fréquents)
MAX_ITERATIONS   = 300   # garde-fou anti-boucle infinie (300 × 7j = ~5,7 ans)
HISTORY_DAYS     = 1825  # 5 ans — volontairement > historique ENEDIS pour tout récupérer
ITER_TIMEOUT     = 1200  # 20 minutes max par itération — tue le subprocess s'il dépasse
...
def _get_max_date_from_csv(csv_path, date_col):
    """Lit la date max d'une colonne date/datetime dans un CSV (streaming, sans pandas)."""
...
def _get_date_from_state(state_path, key):
    """Lit une date ISO depuis le fichier d'état local si elle existe."""
...
# ── CELLULE 4b : Nettoyage local /tmp (évite les résidus des runs précédents) ─
import os
_to_clean = [
...
_required_markers = [
    ...
    "load_curve_max_days_per_run",
    "sync_max_days_per_run",
    "Courbe de charge : plage limitée",
]
...
# ── CELLULE 6 : Boucle de backfill (courbe de charge + consommation) ──────────
...
lc_csv         = f"{LOCAL_TMP}/output/enedis_load_curve.csv"
...
for iteration in range(1, MAX_ITERATIONS + 1):
    lc_last_csv   = _get_max_date_from_csv(lc_csv, "datetime")
    lc_last_state = _get_date_from_state(state_json, "last_load_curve_date")
    lc_last       = _max_iso_date(lc_last_csv, lc_last_state)

    lc_start   = (date.fromisoformat(lc_last)   + timedelta(days=1)).isoformat() if lc_last   else target_start

    lc_done   = lc_start   > target_date

    print(f"── Itération {iteration}/{MAX_ITERATIONS} ──────────────────────────────────────────────")
    print(f"   Courbe 30min : données={lc_last_csv or 'aucune'} | curseur={lc_last_state or 'jamais'} → {'✅ À JOUR' if lc_done   else f'reprise depuis {lc_start}'}")

    if lc_done:
        print(f"\n✅ Backfill complet après {iteration - 1} itération(s) !")
        break

    iteration_log = f"{LOCAL_TMP}/logs/backfill_iteration_{iteration:03d}.log"

    env = os.environ.copy()
    env.update({
        # Courbe de charge : avance par tranches de CHUNK_DAYS
        "LOAD_CURVE_MAX_DAYS_PER_RUN":  str(CHUNK_DAYS),
        "LOAD_CURVE_HISTORY_DAYS":      str(HISTORY_DAYS),
        "ENABLE_LOAD_CURVE":            "true",
        "ENABLE_DAILY_CONSUMPTION":     "false",
        # Pas de collecte customer pendant le backfill (trop lent, pas nécessaire)
        "SKIP_CUSTOMER_COLLECTION":     "1",
        "PYTHONUNBUFFERED":             "1",
        # Désactive les alertes Teams pendant le backfill (évite le spam)
        "ALERT_WEBHOOK_URL":            "",
    ...
    try:
        ...
        with open(iteration_log, "w", encoding="utf-8") as _logf:
            ...
            _logf.write(f"load_curve_resume={lc_start}\n")
    ...
    local_to_onelake(iteration_log,
                     f"{LAKEHOUSE_FILES}/logs/backfill_iteration_{iteration:03d}.log", required=False)
    print()

else:
    print(f"⚠️  Limite de {MAX_ITERATIONS} itérations atteinte — relancer le notebook pour continuer.")

# ── CELLULE 7 : Chargement final dans les Delta tables ───────────────────────
# À lancer après la fin de la boucle.
...
if mssparkutils.fs.exists(f"{LAKEHOUSE_FILES}/output/enedis_data.csv"):
    ...
    print("ℹ️  enedis_data : notebook load curve only — chargement ignoré.")
...
schema_lc = StructType([
    ...
    StructField("unit",             StringType(),    True),
    StructField("interval_length",  StringType(),    True),
    StructField("measure_type",     StringType(),    True),
    StructField("quality",          StringType(),    True),
    StructField("flow_direction",   StringType(),    True),
    StructField("_ingested_at_utc", TimestampType(), True),
])
df_lc = spark.read.option("header","true").schema(schema_lc).csv(
    f"{LAKEHOUSE_FILES}/output/enedis_load_curve.csv")
df_lc = df_lc.withColumn("date",  F.to_date("datetime")) \
             .withColumn("year",  F.year("datetime")) \
             .withColumn("month", F.month("datetime"))
import time as _time

partitions = df_lc.select("year","month").distinct().collect()
n_lc = df_lc.count()
n_parts = len(partitions)
print(f"enedis_load_curve : {n_lc:,} lignes — {n_parts} partition(s) year/month")
for pidx, row in enumerate(partitions, start=1):
    y, m = row.year, row.month
    print(f"  [{pidx}/{n_parts}] Partition year={y} month={m}…", end=" ", flush=True)
    for _attempt in range(3):
        try:
            df_lc.filter((F.col("year")==y)&(F.col("month")==m)) \
                 .write.format("delta").mode("overwrite") \
                 .option("replaceWhere", f"year={y} AND month={m}") \
                 .option("mergeSchema", "true") \
                 .save(f"{LAKEHOUSE_TABLES}/enedis_load_curve")
            print("OK")
            break
        except Exception as _e:
            if "429" in str(_e) or "CapacityLimitExceeded" in str(_e):
                wait = 60 * (_attempt + 1)
                print(f"⏳ Capacité Fabric saturée — pause {wait}s…", flush=True)
                _time.sleep(wait)
            else:
                raise
    else:
        print(f"⚠️  Partition {y}-{m} : échec après 3 tentatives, ignorée.")
    _time.sleep(2)  # petite pause entre partitions pour ne pas saturer la capacité
print(f"✅ enedis_load_curve : {n_lc:,} lignes chargées ({n_parts} partition(s)).")
