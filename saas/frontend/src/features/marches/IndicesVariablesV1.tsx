...
export function IndicesVariablesV1() {
  ...
  const families = useMemo(() => groupByFamily(data?.series ?? []), [data]);
  const familyEntries = Object.entries(families).sort(([a], [b]) => a.localeCompare(b));
  const pointsCount = data?.series.reduce((sum, item) => sum + item.points.length, 0) ?? 0;
