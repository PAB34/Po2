...
export type CpeFinanceImportBatch = {
  ...
  file_size_bytes: number;
  sha256: string;
  target_contract_code: string;
  status: string;
  source_row_count: number;
  imported_line_count: number;
  ignored_line_count: number;
  invoice_count: number;
  matched_site_line_count: number;
  unknown_site_line_count: number;
  missing_site_code_line_count: number;
  created_at: string;
  updated_at: string;
};
...
export type CpeFinanceP1Summary = {
  nb_lignes: number;
  nb_factures: number;
  montant_ht: number;
  types_facture: CpeFinanceGroupSummary[];
  postes_factures: CpeFinanceGroupSummary[];
  nb_lignes_consommation: number;
  nb_lignes_index_releve: number;
  nb_sites_cpe_rapproches: number;
  nb_sites_cpe_avec_pce: number;
  nb_lignes_site_a_reconcilier: number;
};

export type CpeFinanceImportBatchDetail = CpeFinanceImportBatch & {
...
export type CpeFinanceLine = {
  ...
  market: string;
  sold_service: string | null;
  billed_item: string | null;
  amount_ht: number;
  prestation_detail: string | null;
  detected_site_code: string | null;
  site_validation_status: "auto_matched" | "site_unknown" | "site_code_missing" | string;
  consumption: number | null;
  consumption_unit: string | null;
  invoice: CpeFinanceInvoice;
  cpe_site: Pick<CpeSite, "id" | "code_site" | "nom_site" | "pce"> | null;
};
...
export async function fetchCpeFinanceImports(token: string): Promise<CpeFinanceImportBatch[]> {
  const response = await fetch(`${apiBaseUrl}/cpe/finances/imports`, { headers: buildHeaders(token) });
  return parseResponse<CpeFinanceImportBatch[]>(response);
}

export async function fetchCpeFinanceImport(
  token: string,
  batchId: number,
): Promise<CpeFinanceImportBatchDetail> {
  const response = await fetch(`${apiBaseUrl}/cpe/finances/imports/${batchId}`, { headers: buildHeaders(token) });
  return parseResponse<CpeFinanceImportBatchDetail>(response);
}

export async function fetchCpeFinanceLines(
  token: string,
  batchId: number,
  options: { siteValidationStatus?: string; market?: string; limit?: number } = {},
): Promise<CpeFinanceLine[]> {
  const params = new URLSearchParams();
  if (options.siteValidationStatus) params.set("site_validation_status", options.siteValidationStatus);
  if (options.market) params.set("market", options.market);
  if (options.limit) params.set("limit", String(options.limit));
  const query = params.toString();
  const suffix = query ? `?${query}` : "";
  const response = await fetch(`${apiBaseUrl}/cpe/finances/imports/${batchId}/lines${suffix}`, {
    headers: buildHeaders(token),
  });
  return parseResponse<CpeFinanceLine[]>(response);
}

export async function importCpeFinanceExport(
  token: string,
  file: File,
): Promise<CpeFinanceImportBatchDetail> {
  const form = new FormData();
  form.append("file", file);
  const response = await fetch(`${apiBaseUrl}/cpe/finances/imports`, {
    method: "POST",
    headers: { Authorization: `Bearer ${token}` },
    body: form,
  });
